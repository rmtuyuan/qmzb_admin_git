...
(function () {
    ...
    if (urltype == 'add') {
        ...
        $.ajax({
            type: "POST",
            url: "http://gimi321.com/admin.php/tag_admintag",
            data: {},
            dataType: 'JSON',
            cache: false,
            success: function (response) {
                ...
                var tagHtml = '';
                tagHtml +=`<option value="">-请选择-</option>`;
                // 渲染数据
                for (var i = 0; i < tagList.length; i++) {
                    var tagInfo = tagList[i];
                    
                    tagHtml +=`<option value="">${tagInfo.tag}</option>`;
                }
                $('#tag1').html(tagHtml);
                $('#tag2').html(tagHtml);
                $('#tag3').html(tagHtml);
        ...
        $('#postEdit button[type=submit]').click(function (e) {
            // 阻止表单提交
            e.preventDefault();
            console.log($('.img-cover').length);
